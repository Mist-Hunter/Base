#!/bin/sh
# From: https://github.com/bohanyang/debi/blob/master/debi.sh
# shellcheck shell=dash

# NOTE EDITS have been flaged by 'NOTE' see below before you overwrrite!
# Working 7/27/2024
...
while [ $# -gt 0 ]; do
    case $1 in
        ...
        # NOTE edited
        --serial)
            kernel_params="$kernel_params console=tty0 console=ttyS0,115200"
            serial=true
            ;;
        --bbr)
            bbr=true
...
# NOTE edit Add serial console settings if $serial is true
[ "$serial" = "true" ] && {
    $save_preseed << EOF
