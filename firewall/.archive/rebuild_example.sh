...
# Restic
iptables -I OUTPUT -d $REV_PROXY_FQDN -p tcp --dport 2222 -m comment --comment "Debian-Base, up.sh: allow Restic outbound SFTP to reverse proxy" -j ACCEPT
...
iptables -I OUTPUT -p icmp --icmp-type 8 -d 0/0 -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT -m comment --comment "DebSec, firewall.sh: Allow ICMP out, but don't reply"

# Allow Loopback
iptables -I INPUT -i lo -m comment --comment "DebSec, firewall.sh: Allow Loopback" -j ACCEPT
iptables -I OUTPUT -o lo -m comment --comment "DebSec, firewall.sh: Allow Loopback" -j ACCEPT

# Allow existing traffic
iptables -I INPUT -m conntrack --ctstate ESTABLISHED,RELATED -m comment --comment "DebSec, firewall.sh: Allow existing traffic" -j ACCEPT
iptables -I OUTPUT -m conntrack --ctstate ESTABLISHED -m comment --comment "DebSec, firewall.sh: Allow existing traffic" -j ACCEPT

# allow traffic out on port 53 -- DNS
iptables -I OUTPUT -p udp --dport 53 -m comment --comment "DebSec, firewall.sh: allow DNS calls out" -j ACCEPT

# allow traffic out on port 123 -- NTP
iptables -I OUTPUT -m set ! --match-set BOGONS dst -p udp --sport 123 --dport 123 -j ACCEPT -m comment --comment "DebSec, firewall.sh: allow NTP out"
...
# allow DHCP
iptables -I OUTPUT -p udp --sport 67:68 -m comment --comment "DebSec, firewall.sh: allow DHCP" -j ACCEPT
...
#Allow Email
iptables -I OUTPUT -p tcp --dport $SMTP_PORT -j ACCEPT -m comment --comment "DebSec, email.sh: allow Postfix Mail Server, Outbound Mail"

# allow SSH
iptables -I INPUT -p tcp -s $GREEN --dport 22 -m conntrack --ctstate NEW,ESTABLISHED -m comment --comment "DebSec, firewall.sh: allow SSH connections from GREEN" -j ACCEPT
iptables -I OUTPUT -p tcp -d $GREEN --sport 22 -m conntrack --ctstate ESTABLISHED -m comment --comment "DebSec, firewall.sh: allow SSH connections from GREEN" -j ACCEPT
